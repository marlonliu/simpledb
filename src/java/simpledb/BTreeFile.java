...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// If leaf page, get the page and return it.
		if (pid.pgcateg() == BTreePageId.LEAF) {
			return (BTreeLeafPage) (this.getPage(tid, dirtypages, pid, perm));
		}

		// Otherwise it's an internal page.
		BTreeInternalPage page =
			(BTreeInternalPage) (this.getPage(tid, dirtypages, pid, perm));

		// Return the page such that
		// left.key <= f <= right.key (priority to the left child).
		BTreeEntry entry = null;
		Iterator<BTreeEntry> it = page.iterator();
		while (it.hasNext()) {
			entry = it.next();
			if (f == null || f.compare(Op.LESS_THAN_OR_EQ, entry.getKey())) {
				return this.findLeafPage(
								tid,
								dirtypages,
								entry.getLeftChild(),
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		// Create an empty page for the right sibling and move half of the
		// tuples over.
		BTreeLeafPage rightSib =
			(BTreeLeafPage) (this.getEmptyPage(
									  tid,
									  dirtypages,
									  BTreePageId.LEAF));

		// Always round up the number of tuples to move.
		...
		int count = 0;
		Tuple cur = null;
		while (it.hasNext() && count < half) {
			cur = it.next();
			page.deleteTuple(cur);
			rightSib.insertTuple(cur);
			count++;
		}

		// Set the sibling pointers.
		rightSib.setRightSiblingId(page.getRightSiblingId());
		if (page.getRightSiblingId() != null) {
			BTreeLeafPage oldRight =
				(BTreeLeafPage) (this.getPage(
										  tid,
										  dirtypages,
										  page.getRightSiblingId(),
										  Permissions.READ_WRITE));
			oldRight.setLeftSiblingId(rightSib.getId());
		}
		page.setRightSiblingId(rightSib.getId());
		rightSib.setLeftSiblingId(page.getId());

		// Copy the middle key up to the parent.
		Field key = cur.getField(this.keyField);
		BTreeInternalPage parent =
			(BTreeInternalPage) (this.getParentWithEmptySlots(
										  tid,
										  dirtypages,
										  page.getParentId(),
										  field));
		parent.insertEntry(
				   new BTreeEntry(key, page.getId(), rightSib.getId()));
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeInternalPage newPage =
			(BTreeInternalPage) (this.getEmptyPage(
										  tid,
										  dirtypages,
										  BTreePageId.INTERNAL));

		// Always round down the number of entries to move.
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		// Steal from the end of the left child or the beginning of the right child.
		...
		// Redistribute the tuples so that each page is about even.
		int move = (sibling.getNumTuples() - page.getNumTuples()) / 2;
		int count = 0;
		Tuple cur = null;
		while (it.hasNext() && count < move) {
			cur = it.next();
		...
		// Key must be the first element of the right child.
		if (isRightSibling) {
			cur = it.next();
		}

		// Update the key in the parent.
		entry.setKey(cur.getField(this.keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// Calculate the number of entries to move.
		...
		BTreeEntry cur = parentEntry;
		for (int i = 0; i < move; i++) {
			// Get the rightmost entry in left.
			BTreeEntry left = it.next();

			// Move the relevant entry in parent to the right.
			parent.deleteKeyAndRightChild(cur);
			cur.setLeftChild(left.getRightChild());
			cur.setRightChild(right.getLeftChild());
			page.insertEntry(cur);

			// Move the entry in left to the parent.
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry cur = parentEntry;
		for (int i = 0; i < move; i++) {
			// Get the leftmost element in right.
			BTreeEntry right = it.next();

			// Move the relevant entry in parent to the left.
			parent.deleteKeyAndRightChild(cur);
			cur.setLeftChild(left.getRightChild());
			cur.setRightChild(right.getLeftChild());
			page.insertEntry(cur);

			// Move the entry in right to the parent.
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			BTreeLeafPage leftPage, BTreeLeafPage rightPage, BTreeInternalPage parent, BTreeEntry parentEntry) 
					throws DbException, IOException, TransactionAbortedException {
		// Move all of the tuples from the right page to the left page.
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			BTreeInternalPage leftPage, BTreeInternalPage rightPage, BTreeInternalPage parent, BTreeEntry parentEntry) 
					throws DbException, IOException, TransactionAbortedException {
		// Sibling entries of the parent in the new page.
