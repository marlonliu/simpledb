...
public class LogFile {
    ...
    */
    ...
    void writePageData(RandomAccessFile raf, Page p) throws IOException{
        ...
        raf.write(pageData);
        // Debug.log(
        //			 "WROTE PAGE DATA, CLASS = " + pageClassName +
        //			 ", table = " +  pid.getTableId() +
        //			 ", page = " + pid.pageno());
    }
    ...
    Page readPageData(RandomAccessFile raf) throws IOException {
        ...
        try {
            ...
            // Debug.log(
            //			 "READ PAGE OF TYPE " + pageClassName +
            // 			 ", table = " + newPage.getId().getTableId() +
    ...
    */
    ...
    public synchronized void logTruncate() throws IOException {
        ...
        if (cpLoc != -1L) {
            ...
            if (cpType != CHECKPOINT_RECORD) {
                throw new RuntimeException(
                			  "Checkpoint pointer does not point to" +
                			  "checkpoint record");
            }
        ...
        Debug.log(
        		  "TRUNCATING LOG;  WAS " + raf.length() +
        		  " BYTES ; NEW START : " + minLogRecord +
    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                // This is the set of PageIds which have already been reverted.
                HashSet<PageId> reset = new HashSet<PageId>();

                this.raf.seek(this.tidToFirstLogRecord.get(tid.getId()));
                while (this.raf.getFilePointer() < this.raf.length()) {
                	// Get the record type and transaction id.
                	int recordType = this.raf.readInt();
                	long xid = this.raf.readLong();

                	// ABORT, BEGIN, and COMMIT records simply consist of their
                	// type headers. This method is only concerned with UPDATE
                	// records, but CHECKPOINT records must also be handled
                	// separately.
                	if (recordType == LogFile.UPDATE_RECORD) {
                		// Get the before-image.
                		Page before = this.readPageData(this.raf);
                		
                		// If this page was updated by the aborted transaction
                		// and has not yet been reverted, write its
                		// before-image to disk and remove it from the buffer
                		// pool.
                		if (xid == tid.getId() &&
                				!(reset.contains(before.getId()))) {
                			Database
	                			.getCatalog()
	                		...
	                		reset.add(before.getId());
                		}

                		// Read the after-image to move the file pointer.
                		this.readPageData(this.raf);
                	} else if (recordType == LogFile.CHECKPOINT_RECORD) {
                		// Get the number of transactions and seek past all
                		// of them to get to the next record.
                		int numXactions = this.raf.readInt();
                		this.raf.seek(
                					 this.raf.getFilePointer() +
                					 (numXactions * 2 * Long.BYTES));
                	}

                	// Read past the record offset.
                	this.raf.readLong();
                }
            }
        }
    ...
    */
    public synchronized void shutdown() {
        try {
            logCheckpoint();  // simple way to shutdown is to write a
            				  // checkpoint record
            raf.close();
        } catch (IOException e) {
    ...
    */
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                // Seek to the beginning of the log and grab the latest
                // checkpoint.
                this.raf.seek(0);
                long ckpt = this.raf.readLong();

                // Initialize losers set and undo phase start offset. The
                // undo start offset is initialized to just past the checkpoint
                // offset at the beginning of the log.
                HashSet<Long> losers = new HashSet<Long>();
                long undoStart = Long.BYTES;

                // If there exists a checkpoint, get the lowest offset of all
                // active transactions (which will be loser transactions).
                if (ckpt != LogFile.NO_CHECKPOINT_ID) {
                	undoStart = this.raf.length();
                	this.raf.seek(ckpt);
                	this.raf.readInt();
                	this.raf.readLong();
            		int numXactions = this.raf.readInt();
            		for (int i = 0; i < numXactions; i++) {
            			...
            			// Store the offset of the active transaction.
            			this.tidToFirstLogRecord.put(xid, offset);
            		}

            		// Scan past the checkpoint offset.
            		this.raf.readLong();
                }

                // Since we checked for checkpoints above, we do not need to
                // worry about checkpoint records in the log until the undo
                // phase, since both the analysis and redo phases start from
                // the first record after the latest checkpoint.

                // The redo start offset is set to the first record after the
                // checkpoint.
                long redoStart = this.raf.getFilePointer();
                
                // Generate the set of winner and loser transactions.
                HashSet<Long> winners = new HashSet<Long>();
                while (this.raf.getFilePointer() < this.raf.length()) {
                	// Get the record type and transaction id.
                	int recordType = this.raf.readInt();
                	long xid = this.raf.readLong();
                	...
                	if (recordType == LogFile.UPDATE_RECORD) {
                		// Read the before- and after-image to move the file
                		// pointer.
                		this.readPageData(this.raf);
                		this.readPageData(this.raf);
                	} else if (recordType == LogFile.BEGIN_RECORD) {
                		losers.add(xid);
                	} else if (recordType == LogFile.COMMIT_RECORD ||
                				   recordType == LogFile.ABORT_RECORD) {
                		losers.remove(xid);

                		// ABORT records are not losers, but we do not apply
                		// their UPDATEs so we do not include them as winners.
                		if (recordType == LogFile.COMMIT_RECORD) {
                			winners.add(xid);
                		}
                	}

                	// Read the offset and store it if it is a BEGIN record.
                	long offset = this.raf.readLong();
                	if (recordType == LogFile.BEGIN_RECORD) {
                		this.tidToFirstLogRecord.put(xid, offset);
                	}
                }

                // Seek back to the redo start offset for the redo phase.
                this.raf.seek(redoStart);

                // Redo all updates and find all loser transactions.
                while (this.raf.getFilePointer() < this.raf.length()) {
                	// Get the record type and transaction id.
                	int recordType = this.raf.readInt();
                	long xid = this.raf.readLong();
                	...
                	if (recordType == LogFile.UPDATE_RECORD) {
                		// Read the before-image to move the file pointer.
                		this.readPageData(this.raf);

                		// Get the after-image and write it to disk.
                		Page after = this.readPageData(this.raf);

                		// Only write the after page to disk if we are dealing
                		// with a winner transaction.
                		if (winners.contains(xid)) {
	            			Database
	                			.getCatalog()
                ...
                // If there are no loser transactions, we can return at this
                // point.
                if (losers.size() == 0) {
                	return;
                }

                // Seek to either the beginning of the log just past the
                // checkpoint log or to the earliest record of a loser
                // transaction for the undo phase.
                this.raf.seek(undoStart);

                // Undo updates from all loser transactions.
                HashSet<PageId> reset = new HashSet<PageId>();
                while (this.raf.getFilePointer() < this.raf.length()) {
                	// Get the record type and transaction id.
                	int recordType = this.raf.readInt();
    ...
    public void print() throws IOException {
        this.raf.seek(0);
        System.out.println(this.raf.readLong());
    	while (this.raf.getFilePointer() < this.raf.length()) {
        	...
        	case LogFile.UPDATE_RECORD:
        		...
        		break;
        	default:
        		System.out.println("Type not recognized, stopping.");
        		return;
        	}

        	output.append("tid: " + this.raf.readLong());

        	// ABORT, BEGIN, and COMMIT records simply consist of their
        	// type headers. UPDATE and CHECKPOINT records contain more content
        	// and must be handled separately.
        	if (recordType == LogFile.UPDATE_RECORD) {
        		Page before = this.readPageData(this.raf);
        		this.readPageData(this.raf);
        		output.append(
        				  ", [" + before.getId().getTableId() + ", " +
        				  before.getId().pageNumber() + "]");
        	} else if (recordType == LogFile.CHECKPOINT_RECORD) {
        		// Get the number of transactions and seek past all
        		// of them to get to the next record.
        		int numXactions = this.raf.readInt();
        		output.append(", " + numXactions + ": [");
        		for (int i = 0; i < numXactions; i++) {
        			Long xaction = this.raf.readLong();
        			Long offset = this.raf.readLong();
        			output.append("(" + xaction + ", " + offset + "), ");
        		}
        	...
        	output.append(", " + this.raf.readLong() + "\n");
        	System.out.println(output.toString());
        }

    	// Return to current offset or beginning of log.
